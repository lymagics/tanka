...
from tanka.server import Uvicorn
...
async def test_serves_application_over_http():
    ...
    task = asyncio.create_task(
        Tanka(Fixed(Response(Text("from uvicorn"))), Silence()).run(
    ...
    assert_that(
        text,
        equal_to("from uvicorn"),
        "Uvicorn must serve the application on the given port",
    )
